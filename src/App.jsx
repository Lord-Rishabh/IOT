import React, { useState, useEffect } from 'react';
import axios from 'axios';
import './App.css';

function App() {
  const [data, setData] = useState(null);

  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get(
          'https://api.thingspeak.com/channels/2497104/feeds.json?results=1&api_key=JKXFVZRJ6SVQI6A0'
        );

        setData(response.data);
      } catch (error) {
        console.error('Error fetching data:', error);
      }
    };

    fetchData(); // Fetch data immediately when component mounts

    // Setup interval to fetch data every 15 seconds
    const interval = setInterval(fetchData, 15000);

    // Cleanup function to clear interval when component unmounts or changes
    return () => clearInterval(interval);
  }, []);

  if (!data) {
    return <div>Loading...</div>;
  }

  const { channel, feeds } = data;

  return (
    <div className="">
      <div className="container mx-auto px-4 py-8">
        <h1 className="text-4xl font-bold mb-4 text-blue-400">Smart Environmental Monitoring System  </h1>
        <div>
          {feeds.map(feed => (
            <div key={feed.entry_id} className="flex justify-center items-center p-4 rounded-lg mb-4">
              
              <div class="cardContainer">
                <div class="card">
                  <p class="city text-3xl">{channel.name}</p>
                  <p class="weather text-2xl"><span className="font-semibold ">Humidity:</span> {feed.field2}%</p>
                  <svg
                    class="weather"
                    version="1.1"
                    id="Layer_1"
                    x="0px"
                    y="0px"
                    width="150px"
                    height="150px"
                    viewBox="0 0 100 100"
                    xml:space="preserve"
                  >
                    <image
                      id="image0"
                      width="100"
                      height="100"
                      x="0"
                      y="0"
                      href="data:image/png;base64,"
                    ></image>
                  </svg>
                  
                  <p class="temp text-3xl">{feed.field1} °C</p>
                  <div class="minmaxContainer">
                    <div class="min text-2xl">
                      <p class="minHeading">Light</p>
                      <p class="minTemp text-xl">{feed.field3}</p>
                    </div>
                    <div class="max text-2xl">
                      <p class="maxHeading">Gas </p>
                      <p class="maxTemp text-xl">{feed.field4 === "1" ? "No" : "Yes"}</p>
                    </div>
                  </div>
                </div>
              </div>
            </div>

            
          ))}
        </div>
        <a href="https://thingspeak.com/channels/2497104">

        <button
              className="px-4 py-2 neobuttonn rounded-lg  hover:bg-gray-500 focus:outline-none  "
              >View on ThingSpeak</button>
              </a>


      </div>
    </div>
  );
}

export default App;
